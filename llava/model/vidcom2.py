...
def compress_video_tensor(video_tensor, token_per_frame=196):
    ...
    # 计算每个小块的token数量
    chunk_token_size = 16 * token_per_frame
    
    # 将原始Tensor分割成多个小块
    chunks = video_tensor.split(chunk_token_size, dim=0)
    
    # 对每个小块进行压缩处理
    compressed_chunks = []
    for chunk in chunks:
        compressed_chunk = vidcom2_compression(chunk)  # 调用压缩函数
...
def get_indeice_of_select_token_with_conbine_specific_retention(input_tensor, scales,video_mean_score,frame_mean_score,token_per_frame=169):
    ...
    for scale in scales:
        ...
        k_list.append(k)
    combined_similarities = (0.666*frame_mean_score) + (0.333*video_mean_score)
    pruned_indices = []
