...
class PrunableDynamicCache(DynamicCache):
    ...
    def update(
        ...
        if self.kv_cache is None:
            ...
            return torch.gather(self.key_cache[layer_idx], dim=2, index=torch.tensor(self.kv_cache, device=self.key_cache[layer_idx].device).view(1, 1, -1, 1).expand(self.key_cache[layer_idx].size(0), self.key_cache[layer_idx].size(1), -1, self.key_cache[layer_idx].size(3))), torch.gather(self.value_cache[layer_idx], dim=2, index=torch.tensor(self.kv_cache, device=self.value_cache[layer_idx].device).view(1, 1, -1, 1).expand(self.value_cache[layer_idx].size(0), self.value_cache[layer_idx].size(1), -1, self.value_cache[layer_idx].size(3)))
    def update_cache(self, image_attention, config):
        ...
        num_keep = int(img_len * (1 - config.dycoke_ratio))
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, unsqueeze_dim=1):
    ...
    # Adjust position_ids to account for token pruning
    if position_ids[0][0] > sin.size(0):
        position_ids = torch.tensor([[sin.size(0) - 1]])
...
class Qwen2FlashAttention2(Qwen2Attention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_weights = None
        attn_weights = calculate_attention_weights(query_states, key_states, attn_mask=attention_mask)
        return attn_output, attn_weights, past_key_value
...
def calculate_attention_weights(query, key, attn_mask=None, scale=None):
...
class Qwen2SdpaAttention(Qwen2Attention):
    ...
    def forward(
        ...
        # Can be deleted after transformers vision > 5.0
        if position_ids.size(1) == 1 and output_attentions:
            attn_weights = calculate_attention_weights(query_states, key_states, attn_mask=attention_mask)
        else:
            attn_weights = None
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        lengeh_vision_token: Optional[int] = None,
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                #self.attention_score += layer_outputs[1]
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.model(
            ...
            lengeh_vision_token=lengeh_vision_token,
        )
